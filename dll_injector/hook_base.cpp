...
void HookWriter::_writeTheHook()
{
	...
	*reinterpret_cast<uint32_t *>(&jumpToProxyFunction[offset_of_address]) = static_cast<uint32_t>(_proxy_function) - (static_cast<uint32_t>(_injection_addr) + Consts::JUMP_SIZE);
	...
	MemoryProtectionWriteContext memory_write_context(reinterpret_cast<void*>(_injection_addr), Consts::JUMP_SIZE);
...
{	
	...
	unsigned long addressInTargetFunctionAfterJumpToProxy = ((unsigned long)_injection_addr + bytesToBackupCount); // see "position 1" in the README.md in this directory.
